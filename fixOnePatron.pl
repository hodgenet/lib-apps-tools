...
my ($primaryStreetAddressLine1, $primaryStreetAddressLine2, $primaryCountry, $primaryStateOrProvince, $primaryCityOrLocality, $primaryPostalCode, $primaryPhone);
...
if ( defined $q->param("primaryPhone") ) { $primaryPhone = $q->param("primaryPhone"); } else { $primaryPhone = "$Patron_Address_Defaults[6]" }

if ( defined $q->param("recOp") ) { $recOp = $q->param("recOp"); } else { $recOp = '0'; }

@Patron_Address_Defaults = ($primaryStreetAddressLine1,$primaryStreetAddressLine2,$primaryCountry,$primaryStateOrProvince,$primaryCityOrLocality,$primaryPostalCode,$primaryPhone);
...
my ($primaryStreetAddressLine1,$primaryStreetAddressLine2,$primaryCountry,$primaryStateOrProvince,$primaryCityOrLocality,$primaryPostalCode,$primaryPhone);
	...
	$primaryPhone = $Patron_Address_Defaults[6];
...
if ($primaryPhone ne '') {
			$writer->startTag("phone");				# phone if with physical location
				$writer->startTag("number");
					$writer->characters("$primaryPhone");
...
my ($primaryStreetAddressLine1, $primaryStreetAddressLine2, $primaryCountry, $primaryStateOrProvince, $primaryCityOrLocality, $primaryPostalCode, $primaryPhone ) = (@Patron_Address_Defaults);
...
<tr><td colwidth="3"><strong>Phone :</strong> </td><td><input type="text" size="30" name="primaryPhone" value="$primaryPhone" /></td></tr>
</table>
...
## example my @Patron_Address_Defaults = ('My University Library','1111 Some Street','USA','FL','Sarasota','34238''111-2222',);
