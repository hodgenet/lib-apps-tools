...
rename $matchedMARCFile, './Springer_eBookMARC'.$dateTime.'.mrc';					#	rename matched file to timestamped version
...
$matchedMARCFile = ${$_[0]};									#	where to write out the matched/enhanced records
$unmatchedMARCFile = ${$_[1]};								#	source file for brief MARC records
$marcDirectory = ${$_[2]};								#	source file for brief MARC records

($dateTime,$for008Date) = &getDateTimeStamp();
rename $matchedMARCFile, "$marcDirectory".'matched_Springer_eBookMARC'.$dateTime.'.mrc';					#	rename matched file to timestamped version
rename $unmatchedMARCFile, "$marcDirectory".'unmatched_Springer_eBookMARC'.$dateTime.'.mrc';					#	rename matched file to timestamped version
...
my ($this040a,$this0x0a,$this245h,$this338a,$this856u,$this8563,$this6xxi2,@these6xxi2s, $unmatched20a, $matched20a, @these020as,$this001);
...
return ($matched20a,$this040a,$this0x0a,$this245h,$this338a,$this856u,$this8563,@these6xxi2s);
...
my $thisMatch = 0;
my ($unmatched020a,$matched020a,$this040a,$this0x0a,$this245h,$this338a,$this856u,$this8563,@these6xxi2s) = (${$_[0]},${$_[1]},${$_[2]},${$_[3]},${$_[4]},${$_[5]},${$_[6]},${$_[7]},@{$_[8]});
...
if ( ($this856u =~ /springer/i) || ( ($this856u =~ /doi\.org/i) && ($this8563 =~ /springer/i) ) ) {$thisMatch += 10;}
#print "thisMatch : $thisMatch\n";
...
$recoverBatchFile = '/var/www/html/ebookMARC/tmp/recoverBatch.mrc'; $unmatchedBatch = ${$_[0]}; $unmatchedMARCFile = ${$_[2]}; $unmatchedRecord = ${$_[3]};
...
#				my ($matched20a,$matchAgainst040b,$matchAgainst0x0a,$matchAgainst245h,$matchAgainst338a,$matchAgainst856u,$matchAgainst8563,@matchAgainst6xxi2s);
				my ($matched20a,$matchAgainst040b,$matchAgainst0x0a,$matchAgainst245h,$matchAgainst338a,$matchAgainst856u,$matchAgainst8563,@matchAgainst6xxi2s) = getMatchPointsT(\@matchAgainstFields,\$unmatched020a); 		#	 needs @unmatchedFields
				if (doMatch(\$unmatched020a,\$matched20a,\$matchAgainst040b,\$matchAgainst0x0a,\$matchAgainst245h,\$matchAgainst338a,\$matchAgainst856u,\$matchAgainst8563,\@matchAgainst6xxi2s)) {	# check if anything matches
					$matches{$j} = doMatch (\$unmatched020a,\$matched20a,\$matchAgainst040b,\$matchAgainst0x0a,\$matchAgainst245h,\$matchAgainst338a,\$matchAgainst856u,\$matchAgainst8563,\@matchAgainst6xxi2s);


                                my $leader = $matchAgainstRecord->leader();                                        #       get the leader value of the matchAgainstRecord
                                if ( substr($leader,9,1) ne 'a' ) {                                             #       if it's not marked as a unicode record
                                        $matchAgainstRecord = marc8toUtf8(\$matchAgainstRecord);                #       convert it to utf8 and hope for the best
                                }
...
<span class="Breadcrumbs"><a href="http://lib-apps.aus.edu/index.php">Lib-apps Home</a> &gt; Get OCLC Recs for a Springer File</span>

<p><strong><span class="BigRedTitle">AUS Library Get OCLC Recs for a Springer File</span></strong></p>

<p>$opMessage</p>

<form action="getOCLCRecs.pl" method="POST">

<strong>Input Files</strong><br/>
<p>These are the existing MARC Files.  Upload new files from Springer with sftp, select them here for deletion or replacing with OCLC records.</p>
